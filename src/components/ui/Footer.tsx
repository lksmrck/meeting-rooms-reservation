...
const Footer = ({ landingRefs }: any) => {
  ...
  const onClickHandler = (refName: any) => {
  ...
  return (
    <footer className="flex flex-col justify-start  text-white bg-violet-800  shadow-sm mt-auto w-screen  relative ">
      <div className=" border h-full flex flex-col justify-between border-red-600">
        <div className="flex justify-center [&>div]:mx-8 [&>div]:my-8  [&>div]:md:mx-20">
          ...
          <div className="[&>a]:text-xs [&>a]:mt-1 text-gray-300 flex flex-col [&>a]:cursor-pointer">
            ...
            <a
              onClick={() => onClickHandler(landingRefs.featuresRef)}
              id="contactRef"
            >
              Features
            </a>
          ...
          <div className="flex flex-col [&>*]:m-0.5 [&>div]:flex [&>div]:items-center text-gray-300 ">
            <h1 className="mb-1">Social</h1>
            <div>
              <IconButton
                aria-label="twitter"
